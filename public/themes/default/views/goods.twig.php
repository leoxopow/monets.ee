        ...
        <div class="clearfix mb40">
            <div class="col-md-7 col-xs-7 pl0 pr0">
                {% if(Theme.bind('user').id)==1 %}
                    <div class="posa_btns">
                        <div class="btn-group">
                            <button class="btn btn-success" data-toggle="modal" data-target="#edit_goods"><span class="glyphicon glyphicon-pencil"></span></button>
...
{% if(Theme.bind('user').id)==1 %}
<div id="edit_goods" class="modal fade " tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
            ...
            <div class="modal-body">
                ...
                <form action="/updategoods/{{ goods.id }}" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="title" id="good_title" value="{{ goods.title }}">
                    </div>
                    <div class="form-group">
                        <label for="good_price">Цена</label>
                        <input class="form-control" type="text" name="price" id="good_price" value="{{ goods.price }}">
                    </div>
                    <div class="form-group">
                    ...
                    <option value="{{ category.id }}" {% if goods.inCategory(category.id) %}selected {% endif %} >{{category.title}}</option>
                    {%endfor%}
                    </select>
                    </div>
                    <textarea name="description" id="description_textarea" cols="30" rows="10" >{{ goods.description }}</textarea>
